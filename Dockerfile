...
RUN apt-get update && \
    apt-get install -y python3-pip lsb-release curl software-properties-common build-essential debhelper apt-transport-https curl devscripts equivs git-buildpackage && \
    apt-get clean
...
RUN pip install -U bloom pip install pyyaml==5.1 git+https://github.com/ros-infrastructure/ros_buildfarm.git@3.0.1
